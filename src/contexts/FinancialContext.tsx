...
export type FormaPagamento = 'dinheiro' | 'cartao' | 'pix' | 'transferencia' | 'boleto' | 'outro';
export type TipoEntrada = 'Dizimo' | 'Oferta' | 'Campanha' | 'Doacao' | 'Outra Entrada';
...
const FinancialProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addEntrada = useCallback(async (entradaData: Omit<Entrada, 'id' | 'igrejaId' | 'tipo' | 'pago' | 'dataCadastro' | 'ultimaAtualizacao'>): Promise<string | undefined> => {
    ...
    try {
      ...
      if (entradaData.categoria === 'Dizimo' && entradaData.membroId) {
        const dizimoMembroData: Omit<DizimoMembro, 'id'> = {
          ...
          formaPagamento: entradaData.formaPagamento,
  ...
  const calcularSaldos = useCallback((transacoesFiltradas: Transacao[], todasAsTransacoes: Transacao[]): Saldos => {
    ...
    let entradasMes = 0;
    let saidasMes = 0;
    const distribuicao: { [key in TipoEntrada | string]?: number } = {};

    transacoesFiltradas.forEach(t => {
      if (t.tipo === 'entrada') {
        entradasMes += t.valor;
        distribuicao[t.categoria] = (distribuicao[t.categoria] || 0) + t.valor;
    ...
    const distribuicaoReceitasArray = Object.entries(distribuicao)
      .map(([categoria, valor]) => ({
        categoria: categoria as TipoEntrada,
        valor: valor || 0,
